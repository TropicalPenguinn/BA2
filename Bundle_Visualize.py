...
if __name__ == "__main__":
    ...
    # Set image path
    img_path=['./desk2/data/align_test{}.png'.format(i) for i in range(1,14)]
    depth_path=['./desk2/data/align_test_depth{}.png'.format(i) for i in range(1,14)]
    pcd=[o3d.io.read_point_cloud('./desk2/pcd/result{}.pcd'.format(i)) for i in range(1,14)]
...
####################################################################################################################################################################
    ...
    #Get relative Pose
    relative_pose=[(get_relative_pose(sift_result[i],sift_result[i+1],boundary[i],boundary[i+1],depth[i],depth[i+1],image[i],image[i+1],pcd[i],pcd[i+1])) for i in range(0,len(image)-1)]
    """
...
#Scipy Optimization
    ...
    points_proj, points_2d = fun2(res.x, n_cameras, n_points, camera_indices, point_indices, points_2d)
    ...
    for i in range(covisibility_graph.shape[1]):
        ...
        cv2.imwrite('result_bundle{}.png'.format(i), img)
    ...
    for i in range(n_cameras):
        ...
        if result==None:
            ...
            copy2=copy.deepcopy(pcd[i])
            result=copy1.transform(trans1)
            result2=copy2
            pre=copy.deepcopy(pcd[i])
        else:
            copy1=copy.deepcopy(pcd[i])
            copy2=copy.deepcopy(pcd[i])
            copy3=copy.deepcopy(pcd[i])

            result+=copy1.transform(trans1)
            result2=result2.transform(relative_pose_forward[i-1])+copy2
            result3=pre.transform(relative_pose_forward[i-1])+copy3
            pre=copy.deepcopy(pcd[i])
